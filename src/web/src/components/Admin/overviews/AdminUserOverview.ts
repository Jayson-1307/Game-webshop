 ...
 *
 * @todo Most of the logic in this component is over-simplified. You will have to replace most of if with actual implementations.
 */
@customElement("admin-users-overview")
export class AdminUserOverview extends LitElement {
    ...
    /**
     * Get all users from database (only the accounts with authorization level "user" are shown)
     */
    ...
    public render(): TemplateResult {
        return html`
            <h2>User Overview</h2>
            <div class="filters">
                <button class="create-button" @click=${this.navigateToCreateAccount}>Create User</button>
                ...
                <tbody>
                    ${this._users.map(user => html`
                        <tr>
                            ...
                            <td>${user.email}</td>
                            <td class="actions">
                                <button class="edit" @click=${(): void => this.navigateTo(RouterPage.EditAccount, user.id)}>
                                    Edit
                                </button>
                            </td>
                        </tr>
